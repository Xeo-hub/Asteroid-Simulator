...
void generate_space(struct space_soa &space, int random_seed, int num_objects, double size_enclosure) {
    ...
    /* Paralelizamos la inicialización de velocidades, un hilo cada uno para evitar problemas*/
...
void calculate_collisions(struct space_soa &space, int num_objects){
...
void calculate_forces(struct space_soa &space, int num_objects) {
    /* Un vector que almacena las fuerzas */
    double fuerza[3];
    /* Un vector que nos ayuda con las distancias */
    double aux_vector[3];
    /* Recorremos los objetos */
    #pragma omp parallel for schedule(guided) private(fuerza, aux_vector) num_threads(4)
    for (int i = 0; i < num_objects; i++) {
        /* Si no han colisionado */
        if (0!=space.vec_m[i]) {
...
int main(int argc, char *argv[]) {
    ...
    if (!outfile_initial){cerr<<"Error"; exit(-1);}
        ...
        calculate_collisions(space, num_objects);
        for (int i=0; i<num_objects; i++){
